...
const verifyKakaoURL = 'https://kauth.kakao.com/oauth';
...
export const verifyKakao = async (code: string) => {
  ...
  if (resData) {
    ...
    const userData = await getUserInfoFromKakao(access_token);
    if (userData) {
      return userData;
    }
  }
...
const getUserInfoFromKakao = async (token: string) => {
  let resData;
  try {
    const response = await axios.get(`${verifyKakaoURL}/token`, {
      headers: {
